...
def min_difference(root,k):
    ...
    return abs(int(closest-k))
def getClosestNode(tree,target,closest):
    if tree == None:
        return closest
    if abs(closest-target)>abs(tree.data-target):
        closest=tree.data
    if target>tree.data:
        return getClosestNode(tree.right,target,closest)
    elif target< tree.data:
        return getClosestNode(tree.left,target,closest)
    else:
        return target
